import React from "react";
import { Link } from "react-router-dom";
import axios from "axios";
import useSWR from "swr";

const ProductList = () => {
    const fatcher = async () =>{
        const response = await axios.get('http://localhost:5000/user/')
        return response.data;
    }

    const {data}= useSWR('user',fatcher)
    if(!data) return <h2>Loading..</h2>



  return (
    <div className=" flex flex-col mt-5">
      <div className="w-full">
        <Link
          to="/add"
          className="bg-blue-500 hover:bg-blue-700 border
             border-slate-200 text-white font-bold py-2 px-4 rounded-lg"
        >
          add new product
        </Link>
        <div className="relative shadow rounded-lg mt-3"></div>
        <table className="w-full text-sm text-left text-gray-500">
          <thead className="text-xs text-gray-700 uppercase bg-gray-100">
            <tr>
              <th className=" py-3 px-1 text-center">No</th>
              <th className=" py-3 px-6 ">User Name </th>
              <th className=" py-3 px-6 ">password</th>
              <th className=" py-3 px-6 ">alamat</th>
              <th className=" py-3 px-1 text-center">Action</th>
            </tr>
          </thead>
          <tbody>
            {data.map((user, index)=>(
                <tr className="bg-white border-b" key={user.id}>
              <td className=" py-3 px-1 text-center">{index+1}</td>
              <td className=" py-3 px-6 font-medium text-gray-900">
                {user.name}
              </td>
              <td className=" py-3 px-6">{user.password}</td>
              <td className=" py-3 px-6">{user.alamat}</td>
              <td className=" py-3 px-1 text-center">
                <Link
                  to={`/edit/${user.id}`}
                  className="font-medium bg-green-400 hover:bg-green-700 px-3 py-1 rounded-lg text-white mr-1"
                >
                  edit
                </Link>
                <button className="font-medium bg-gray-400 hover:bg-gray-700 px-3 py-1 rounded-lg text-white mr-1">
                  Delete
                </button>
              </td>
            </tr>
            ))}
            
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default ProductList;
